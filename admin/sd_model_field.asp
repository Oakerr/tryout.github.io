...
''' 修改：IT平民 in 2014.08
	...
	sub adddb()
		...
		dim mdata:mdata=sdcms.db.dbload(1,"tablename","sd_model","modelid="&modelid&"","modelid")
		...
		dim data
		data=array(array("fname",t0,0),array("falias",t1,30),array("fclass",t2,0),array("fmode",t3,0),array("fdatatype",t4,0),array("flength",t5,5),array("fdefault",t6,255),array("foptions",t7,0),array("fismust",t8,0),array("frules",t9,0),array("fistrim",t10,0),array("ordnum",t11,0),array("islock",t12,0),array("fupload",t13,0),array("modelid",modelid,0))
		if sdcms.db.dbnew("sd_model_field",data,"fname='"&t0&"' and modelid="&modelid&"")=0 then
	...
	sub editdb()
		...
		data=array(array("falias",t1,30,1),array("fdefault",t6,255,1),array("foptions",t7,0,1),array("fismust",t8,0,0),array("frules",t9,0,0),array("fistrim",t10,0,0),array("ordnum",t11,0,0),array("islock",t12,0,0),array("fupload",t13,0,0))
		if sdcms.db.dbupdate("sd_model_field","id="&id&"",data)=1 then
			sdcms.ajaxjson "保存成功",1
		end if
	end sub
	...
	sub orderdb()
		...
		for i=0 to ubound(t2)
			if sdcms.isnum(t2(i)) then
				sdcms.db.dbupdate "sd_model_field","id="&t3(i)&"",array(array("ordnum",t2(i),10,0,0))
			end if
		next
		sdcms.ajaxjson "保存成功",1
	end sub
	...
	sub deldb()
		...
		dim data:data=sdcms.db.dbload(1,"modelid,fname","sd_model_field","id="&id&"","")
		if ubound(data)<0 then
			exit sub
		end if
		dim mdata:mdata=sdcms.db.dbload(1,"tablename","sd_model","modelid="&data(0,0)&"","modelid")
		if ubound(mdata)<0 then
			exit sub
		end if
		sdcms.db.dbdel "sd_model_field","id="&id
	...
	sub delcache()
		sdcms.delcache "model_field"&modelid
	end sub
	...
	dim modelid:modelid=sdcms.getint(sdcms.fget("modelid",0),0)
	select case act
		case "add":load theme_model_field_add
		case "adddb":adddb:delcache
		case "edit"
			dim datadb,id
			id=sdcms.getint(sdcms.fget("id",0),0)
			datadb=sdcms.db.dbload("","fname,falias,fclass,fmode,fdatatype,flength,fdefault,foptions,fismust,frules,fistrim,ordnum,islock,fupload","sd_model_field","id="&id&"","")
			load theme_model_field_edit
		case "editdb":editdb:delcache
		case "order":orderdb:delcache
		case "del":deldb:delcache
		case else:load theme_model_field
	end select
