...
''' 修改：IT平民 in 2014.07
	...
	sub adddb()
		...
		dim t10,t11,t12,t13,t14,t15,t16,t17
		dim o0,o1,o2,o3
		...
		if sdcms.strlen(t17)>0 then
			if not(sdcms.checkstr(t17,"urlname")) then
				sdcms.ajaxjson "文件名称错误，不能包含特殊字符",0:exit sub
			end if
			if sdcms.db.dbcount("sd_content","url='"&t17&"'")>0 then
	...
	sub editdb()
		dim t0,t1,t2,t3,t4,t5,t6,t7,t8,t9
		dim t10,t11,t12,t13,t14,t15,t16,t1_0,t17
		dim o0,o1,o2
		dim s0,s1,s2,style,up,frist
		...
		if sdcms.strlen(t17)>0 then
			if not(sdcms.checkstr(t17,"urlname")) then
				sdcms.ajaxjson "0文件名称错误，不能包含特殊字符",0:exit sub
			end if
			if sdcms.db.dbcount("sd_content","url='"&t17&"' and id<>"&id&"")>0 then
				sdcms.ajaxjson "0文件名称已存在，请更换！",0:exit sub
			end if
		end if
		if up=1 then
		...
		fstr="array(array(""content"",t2,0,1)"&fstr&")"
		sdcms.db.dbupdate "sd_model_product","cid="&id&"",eval(fstr)
	...
	select case act
		...
		case "edit","publish"
			...
			likeid=data(10,0)
			dim rsshow
			set rsshow=sdcms.db.exedb("select top 1 * from sd_model_product where cid="&id&"")
			if rsshow.eof then
