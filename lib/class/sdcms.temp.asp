...
class sdcms_template
	...
	private loop_eof,loop_head,loop_foot,loop_subject
	...
	private sub analysis_block()
		...
		if matches.count>0 then
			for each match In matches
				dim mt_root:mt_root=theme_root
				if ismobile then mt_root=replace(mt_root,"/mobile","")
				templatedata=replace(templatedata,match.value,getfile(webroot&"theme/"&mt_root&"/block/"&match.submatches(0)&".asp"))
			next
		end if
	end sub
		...
		if matchs.count>0 then
			for each match In matchs
				...
				if tag_pagesize<>0 then
					...
					if len(loop_head)>0 then
						t1=t1&texttoasp("if not "&t8&".eof then")
						t1=t1&loop_head
						t1=t1&texttoasp("end if")	
					end if
					...
					if len(loop_foot)>0 then
						t2=t2&texttoasp("if not "&t8&".eof then")
						t2=t2&loop_foot
						t2=t2&texttoasp("end if")	
					end if
	...
	private sub get_tag(byval t0)
		...
		if tag_field<>"" then
			reg.pattern="\<([a-zA-Z0-9_().]*)\>"
			tag_field=reg.replace(tag_field,"""&$1&""")
		end if
		if tag_table<>"" then
			reg.pattern="\[([a-zA-Z0-9_().]*)\]"
	...
	private sub getloop(byval t0,byval t1)
		...
		reg.pattern="{"&t1&":foot}([\s\S]+?){/"&t1&":foot}"
		set matchess=reg.execute(t0)
		if matchess.count>0 then
			for each matchs In matchess
				loop_foot=matchs.submatches(0)
				t0=reg.replace(t0,"")
			next
		else
			loop_foot=""
